	<form action="{$data.current.url}" id="resetPasswordForm" method="post">
		<fieldset>
			...
			{include file='common/formFields/user/password.tpl' name='userNewPassword' label="{t}password{/t}" pattern={$data._columns.users.password.pattern} placeholder="{t}new password{/t}" autocomplete=false hint={$data._columns.users.password.hint}}
