<form action="{$data.current.url}" id="resetPasswordForm" method="post">
	<fieldset>
		...
		{include file='common/formFields/user/password.tpl' name='userNewPassword' label="{t}new password{/t}" pattern={$data.dataModel.users.password.pattern} placeholder="{t}new password{/t}" autocomplete=false hint={$data.dataModel.users.password.hint}}
