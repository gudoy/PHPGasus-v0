...
{else}
	...
	<option value="{$item[$field.relField]}" {if (isset($smarty.post.$resourceFieldName) && $smarty.post.$resourceFieldName === $item[$field.relField]) || $resource[$fieldName] === $item[$field.relField]}selected="selected"{/if}>{$item.id} - {$item.title}</option>
	{/foreach}
{/if}
</select>