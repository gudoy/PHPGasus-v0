...
{else}
	...
	{/if}
{/if}
<span class="icon inputIcon"></span><input type="email" name="{$resourceFieldName}{$useArray}" id="{$resourceFieldName}{$itemIndex}"{if $field.length} maxlength="{$field.length}"{/if} class="normal {if $field.check}check-{$field.check}{/if}" value="{$value}" {if $field.pk || !$editable || ($mode === 'create' && $field.computed && (!isset($field.forceEditable) || $field.forceEditable != true))}disabled="disabled"{/if}{if $isRequired} required="required"{/if} />
{/if}