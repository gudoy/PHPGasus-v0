...
<section class="accountConfirmation" id="accountConfirmation">
	...
	<header>
		<h2 class="title">{t}Account Confimed!{/t}</h2>
	</header>
	<div class="content">
		...
		<nav class="actions">
			{include file='common/blocks/actionBtn.tpl' href={$smarty.const._URL_HOME} id='goToHomeBtn' class='action goToHomeBtn primary' label="{t}back to home{/t}"}<span class="or">{t}or{/t}</span>
			{include file='common/blocks/actionBtn.tpl' href={$smarty.const._URL_LOGIN} id='goToLoginBtn' class='action goToLoginBtn secondary' label="{t}login{/t}"}
		</nav>
	</div>
