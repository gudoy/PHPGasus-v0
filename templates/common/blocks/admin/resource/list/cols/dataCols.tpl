...
<td id="{$fieldName}Col{$resource.id}" class="col {$fieldName}Col typeInt fk{if $isSorted} activeSort{/if}{if !$displayed} hidden{/if}" headers="row{$resource.id} {$fieldName}Col">
...
<td id="{$field.relGetAs}Col{$resource.id}" class="col {$field.relGetAs}Col typeVarchar fk{if $isSorted} activeSort{/if}{if !$displayed} hidden{/if}" headers="row{$resource.id} {$field.relGetAs}Col">
...
<td id="{$fieldName}Col{$resource.id}" class="col dataCol {$fieldName}Col type{$field.type|ucfirst}{if $field.subtype} subtype{$field.subtype|ucfirst}{/if}{if $isDefaultNamefield} defaultNameField{/if}{if $isSorted} activeSort{/if}{if $field.relResource} typeRel{/if}{if !$displayed} hidden{/if}" headers="row{$resource.id} {$fieldName}Col">
