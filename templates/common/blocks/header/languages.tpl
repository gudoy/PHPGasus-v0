...
<ul class="menu nav chooser languagesMenu" id="languagesMenu">
	<li class="secondary {if $curLang === 'fr'}current{/if} first"><a href="{$smarty.const._URL}{$cleanedURI}{if strpos($cleanedURI, '?') > -1}&amp;{else}?{/if}lang=fr_FR" lang="fr-FR" title="fr-FR"><span class="locale">FR</span> <span class="value">{t}French{/t}</span></a>
	</li><li class="secondary {if $curLang === 'en'}current{/if} last"><a href="{$smarty.const._URL}{$cleanedURI}{if strpos($cleanedURI, '?') > -1}&amp;{else}?{/if}lang=en_US" lang="en-US" title="en-US"><span class="locale">EN</span> <span class="value">{t}English{/t}</span></a>
	</li>
</ul>
</div>
{/if}