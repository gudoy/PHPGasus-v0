...
{/if}
		...
		<input type="{$type}" class="normal{if $required} check-required{/if}" name="{$postValName}" id="{$postValName}"{if $type !== 'password' && $value || $smarty.post[$postValName]} value="{$value|default:$smarty.post[$postValName]}"{/if}{if $required} required="required"{/if}{if $placeholder} placeholder="{$placeholder}"{/if}{if $autofocus} autofocus="autofocus"{/if}{if $disabled} disabled="disabled"{/if}{if $autocomplete} autocomplete="{if $autocomplete}on{else}off{/if}"{/if} />
{if !isset($inputOnly) || !$inputOnly}
	</div>
</div>
{/if}