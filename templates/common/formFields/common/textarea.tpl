...
<div class="line">
	...
	<div class="fieldBlock">
		<textarea class="normal {if $required}check-required{/if}" name="{$postValName}" id="{$postValName}" {if $required && $html}required="required"{/if} cols="{$cols|default:80}" rows="{$rows|default:4}">{$smarty.post[$postValName]|stripslashes|trim}</textarea>
	</div>
</div>