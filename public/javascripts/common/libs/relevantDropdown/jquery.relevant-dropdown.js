...
$.fn.relevantDropdown = function(options)
{
	...
	return this.each(function()
	{
		...
		// Prevent conflits with browser default autocomplete
		// but if we put it the base markup, native datalist is broken on Opera 
		$input.attr(
		{
			autocomplete:'off'
		});
		...
		$input
			...
			datalistItems
				 .on("mouseenter", function() { $(this).addClass("active").siblings().removeClass("active"); })
			...
			{
Tools.log('keycode: ' + e.keyCode);
				
				e.stopPropagation();
				
				var //active 				= $("li.active"),
					active 				= $datalist.find("li.active"),
				...
				else if ( e.keyCode == 40 )
				{
					if (active.length)
					{
        				...
        				{
							$datalist.stop().animate({scrollTop: scrollValue += datalistItemsHeight}, 200);
						}                    
					}
					else
					{			    
				...
				else if ( e.keyCode == 13 || e.keyCode == 9 )
				{
					if (active.length)
					{
						// Prevent the parent form to be submited while we just want to select/validate the active item 
						e.preventDefault();
						e.stopPropagation();
						...
						//item_selected(active.data('value'));
			...
			datalistItems.on("click", function(e)
			{
				...
				//item_selected($(this).data('value'));
			});

			function item_selected(new_text)
			{
				if( typeof options.change === 'function' ) { options.change.call(this, new_text); }
			}
