...
function do_file($file)
{
	...
	// 
	### Start addition by doyer.guyllaume@gmail.com ### 
	// The following patterns look for the following gettext modifiers syntax
	// {'sample enclosed in simple quotes'|gettext}
	// {"sample enclosed in quotes quotes"|gettext}
	// {sample without any quotes|gettext}
	// {assign var=foo value='sample prefixed by a equal'|gettext}
	// {assign var=foo value="sample prefixed by a equal"|gettext}
	// {assign var=foo value=sample prefixed by a equal|gettext} <== TODO
	// If you find a cleaner way to do this, let me know!!
	preg_match_all(
			'/\{(.*)\=?(\'|\")?(.*)(\\1|\\2)\|gettext.*\}/',
			$content,
			$matchesGTmodifiers
	);
	### End addition by doyer.guyllaume@gmail.com ###
	...
	### Start addition by doyer.guyllaume@gmail.com ###
	for ($i=0; $i < count($matchesGTmodifiers[0]); $i++)
	{
		echo '<?php _("'.fs($matchesGTmodifiers[3][$i]).'"); ?>'."\n\n";
	}
	### End addition by doyer.guyllaume@gmail.com ###
}
